...
void NiawgCore::setDefaultWaveforms() {
	debugMsg += "setDefaultWaveforms: Starting\n";
	try {
		if (defaultWaveName.empty()) {
			debugMsg += "Writing default waveforms\n";
			NiawgOutput expOutput;
			std::vector<parameterType> params; // Empty params for default
			writeStaticNiawg(expOutput, params, 0, 1, false, niawgLibOption::defaultMode);
			debugMsg += "Default waveforms written: name=" + defaultWaveName + ", size=" +
				str(defaultMixedWaveform.size()) + "\n";
		}
		else {
			debugMsg += "Default waveforms already set: name=" + defaultWaveName + "\n";
		}
		// ... (rest of the function) ...
	}
	catch (ChimeraError & e) {
		debugMsg += "ERROR in setDefaultWaveforms: " + std::string(e.what()) + "\n";
		throwNested("FATAL ERROR: Analysis of Default Waveforms and Default Script Has Failed.\n" + std::string(e.what()));
	}
}
...
void NiawgCore::finalizeStandardWave(simpleWaveForm& wave, unsigned varnum, std::vector<parameterType>& variables,
	...
	// Only set name if empty; preserve script-provided names
	if (wave.name.empty()) {
		wave.name = "wave" + str(varnum) + str(std::time(nullptr));
		debugMsg += "Set default unique name: " + wave.name + "\n";
	}
	else {
