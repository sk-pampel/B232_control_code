...
class DaqMxFlume {
public:
	// Existing constructor and members...
	DaqMxFlume(bool safemodeOption);
	...
	void createTask(const char* taskName, TaskHandle& handle);
	void readAnalogF64(TaskHandle taskHandle, std::vector<float64>& readData, int32& sampsPerChanRead);
	void createAiVoltageChan(TaskHandle taskHandle, const char physicalChannel[], const char nameToAssignToChannel[],
		int32 terminalConfig, float64 minVal, float64 maxVal, int32 units, const char customScaleName[]);
	void createAoVoltageChan(TaskHandle taskHandle, const char physicalChannel[], const char nameToAssignToChannel[],
		float64 minVal, float64 maxVal, int32 units, const char customScaleName[]);
	void createDiChan(TaskHandle taskHandle, const char lines[], const char nameToAssignToLines[], int32 lineGrouping);
	void stopTask(TaskHandle handle);
	void configSampleClkTiming(TaskHandle taskHandle, const char source[], float64 rate, int32 activeEdge,
		int32 sampleMode, uInt64 sampsPerChan);
	void writeAnalogF64(TaskHandle handle, int32 numSampsPerChan, bool32 autoStart, float64 timeout,
	...
	bool isTaskRunning(TaskHandle handle);
	void ensureTaskStopped(TaskHandle handle);
