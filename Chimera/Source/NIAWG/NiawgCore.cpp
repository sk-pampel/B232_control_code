...
void NiawgCore::loadStandardInputFormType(std::string inputType, channelWaveForm& wvInfo) {
	...
	for (auto number : range(NiawgConstants::MAX_NIAWG_SIGNALS)) {
		...
		else if (inputType == "gen" + num_s + "sinmod" || inputType == "gen" + num_s + "sinmod_v") {
			wvInfo.initType = number + 4 * NiawgConstants::MAX_NIAWG_SIGNALS;
		}
		else if (inputType == "gen" + num_s + "rar" || inputType == "gen" + num_s + "rar_v") {
			wvInfo.initType = number + 5 * NiawgConstants::MAX_NIAWG_SIGNALS;
		}
		else if (inputType == "gen" + num_s + "tosinmod" || inputType == "gen" + num_s + "tosinmod_v") {
...
void NiawgCore::calcWaveData(channelWaveForm& inputData, std::vector<ViReal64>& readData, long int sampleNum,
	...
	for (unsigned signal = 0; signal < inputData.waveSigs.size(); signal++) {
		...
		if (inputData.waveSigs[signal].freqRampType == "doublesinmod") {
			...
			deltaOmega.push_back(2 * PI * dNu);
			accel_w0.push_back(0);
			accel_w1.push_back(4 * PI * dNu / t_r - accel_w0.back());
			jerk.push_back(8 * PI * dNu / (t_r * t_r) - 4 * accel_w0.back() / t_r);
			freq_1.push_back(f_0 + dNu / 2);
			phi_halfway.push_back(0.5 * accel_w0.back() * (t_r2 * t_r2) + (t_r2 / 6.0) * (2 * PI * dNu - accel_w0.back() * t_r)
	...
	for (; sample < sampleNum; sample++) {
		...
		for (auto signal : range(inputData.waveSigs.size())) {
			...
			//}
				...
				double f_mod = modFreq[signal];
				double A_mod = modAmp[signal];
				phasePos[signal] = 2 * PI * f_0 * t - (A_mod / f_mod) * cos(2 * PI * f_mod * t) + (A_mod / f_mod) + phi_0;
			}
			else if (inputData.waveSigs[signal].freqRampType == "tosinmod") {
				double f_mod = modFreq[signal];
				double A_mod_final = modAmp[signal];  // Final amplitude of frequency oscillation
				double t_normalized = t / waveTime;
				double A_mod = A_mod_final * (tanh(-4 + 8 * t_normalized) + 1) / 2;
				phasePos[signal] = 2 * PI * f_0 * t - (A_mod / f_mod) * cos(2 * PI * f_mod * t) + (A_mod / f_mod) + phi_0;
			}
			else if (inputData.waveSigs[signal].freqRampType == "fromsinmod") {
				double f_mod = modFreq[signal];
				double A_mod_final = modAmp[signal];  // Final amplitude of frequency oscillation
				double t_normalized = t / waveTime;
	...
	for (auto signal : range(inputData.waveSigs.size())) {
		...
		//}
			...
			double f_mod = modFreq[signal];
			double A_mod = modAmp[signal];
			phasePos[signal] = 2 * PI * f_0 * curTime - (A_mod / f_mod) * cos(2 * PI * f_mod * curTime) +
				(A_mod / f_mod) + phi_0;
		}
		else if (inputData.waveSigs[signal].freqRampType == "tosinmod") {
			double f_mod = modFreq[signal];
			double A_mod_final = modAmp[signal];  // Final amplitude of frequency oscillation
			double t_normalized = curTime / waveTime;
			double A_mod = A_mod_final * (tanh(-4 + 8 * t_normalized) + 1) / 2;
			phasePos[signal] = 2 * PI * inputData.waveSigs[signal].freqInit.getValue(varNum) * 1e6 * curTime
...
void NiawgCore::readTraditionalSimpleWaveParams(ScriptStream& script, std::vector<parameterType>& parameters,
	...
	for (int signal = 0; signal < int(wave.chan[axis].waveSigs.size()); signal++) {
		...
		case 5: { // genXrar
			script >> sig.freqInit >> sig.powerRamp.start >> sig.powerRamp.end >> sig.powerRamp.releaseTime >> sig.initPhase;
			sig.freqRampType = "nr";
			sig.powerRamp.type = "rar";
			sig.freqFin = sig.freqInit;
			assertAllValid(sig, parameters);
			sig.freqInit.internalEvaluate(parameters, 1);
			sig.powerRamp.start.internalEvaluate(parameters, 1);
			sig.powerRamp.end.internalEvaluate(parameters, 1);
			sig.powerRamp.releaseTime.internalEvaluate(parameters, 1);
			sig.initPhase.internalEvaluate(parameters, 1);
			break;
		}
		case 6: { // genXtosinmod (ramping up to sinmod)
			script >> sig.freqInit >> sig.freqFin >> sig.powerRamp.start >> sig.modFreq >> sig.initPhase;
			sig.freqRampType = "tosinmod";
			sig.powerRamp.end = sig.powerRamp.start;
			sig.powerRamp.type = "nr";
			assertAllValid(sig, parameters);
			break;
		}
		case 7: { // genXfromsinmod (ramping down from sinmod)
			script >> sig.freqInit >> sig.freqFin >> sig.powerRamp.start >> sig.modFreq >> sig.initPhase;
...
void NiawgCore::readVectorizedSimpleWaveParams(ScriptStream& script, std::vector<vectorizedNiawgVals>& constVecs,
	...
	case 4: { // genXsinmod_v
		vectorizedNiawgVals initFreqs, modAmps, powers, modFreqs, phases;
		script >> initFreqs.name >> modAmps.name >> powers.name >> modFreqs.name >> phases.name;
		for (auto& cv : constVecs) {
			if (cv.name == initFreqs.name) initFreqs = cv;
	...
	case 5: { // genXrar_v
		vectorizedNiawgVals freqs, initAmps, finAmps, releaseTimes, phases;
		script >> freqs.name >> initAmps.name >> finAmps.name >> releaseTimes.name >> phases.name;
		for (auto& cv : constVecs) {
			if (cv.name == freqs.name) freqs = cv;
			if (cv.name == initAmps.name) initAmps = cv;
			if (cv.name == finAmps.name) finAmps = cv;
			if (cv.name == releaseTimes.name) releaseTimes = cv;
			if (cv.name == phases.name) phases = cv;
		}
		auto numSigs = wave.chan[axis].waveSigs.size();
		if (freqs.vals.size() == 0) thrower("Failed to find constant vector named " + freqs.name);
		if (initAmps.vals.size() == 0) thrower("Failed to find constant vector named " + initAmps.name);
		if (finAmps.vals.size() == 0) thrower("Failed to find constant vector named " + finAmps.name);
		if (releaseTimes.vals.size() == 0) thrower("Failed to find constant vector named " + releaseTimes.name);
		if (phases.vals.size() == 0) thrower("Failed to find constant vector named " + phases.name);
		if (freqs.vals.size() != numSigs) thrower("Vector " + freqs.name + " size mismatch!");
		if (initAmps.vals.size() != numSigs) thrower("Vector " + initAmps.name + " size mismatch!");
		if (finAmps.vals.size() != numSigs) thrower("Vector " + finAmps.name + " size mismatch!");
		if (releaseTimes.vals.size() != numSigs) thrower("Vector " + releaseTimes.name + " size mismatch!");
		if (phases.vals.size() != numSigs) thrower("Vector " + phases.name + " size mismatch!");
		for (auto signal : range(numSigs)) {
			auto& sig = wave.chan[axis].waveSigs[signal];
			sig.freqInit = freqs.vals[signal];
			sig.freqFin = sig.freqInit;
			sig.initPhase = phases.vals[signal];			
			sig.powerRamp.start = initAmps.vals[signal];
			sig.powerRamp.end = finAmps.vals[signal];
			sig.powerRamp.releaseTime = releaseTimes.vals[signal];
			sig.powerRamp.type = "rar";
			sig.freqRampType = "nr";
			assertAllValid(sig, parameters);
			sig.freqInit.internalEvaluate(parameters, 1);
...
double NiawgCore::rampCalc(int totalSamples, int iteration, const rampInfo& ramp, unsigned varNum) {
	...
	else if (ramp.type == "rar") {
		double time = (double)iteration / NiawgConstants::NIAWG_SAMPLE_RATE;  // Time in seconds
		double releaseTime = ramp.releaseTime.getValue(varNum) / 1000.0;     // ms to s
		if (time < releaseTime) {
			return rampRange;
		}
		return 0;
