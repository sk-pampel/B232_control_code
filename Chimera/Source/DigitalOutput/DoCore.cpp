...
void DoCore::calculateVariations (std::vector<parameterType>& params, ExpThreadWorker* threadworker){
	...
	for (auto variationNum : range (variations)){
		...
		for (auto& dioCommandForm : ttlCommandFormList){
			...
			qDebug() << ">>Variation number " << variationNum << ", at commnad number " << cnt << ", finished at time = " << timer.elapsed();
			cnt++;

		}
		qDebug() << "Variation number " << variationNum << ", finished at time = " << timer.elapsed();
	}
	restructureCommands ();
}
