...
class AiSystem : public IDeviceCore {
		public:
			...
			std::vector<float64> getResVals() const { return resVals; }
			std::vector<float64> getCtrlVals() const { return ctrlVals; }
		...
		private:
			...
			DaqMxFlume daqmx;
			std::vector<float64> resVals; // Store result values
			std::vector<float64> ctrlVals; // Store control values
			std::mutex aiMutex; // Add mutex
