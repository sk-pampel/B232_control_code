...
struct niawgWavePower {
	enum class mode { constant, capped, unrestricted };
...
struct niawgLibOption {
	enum class mode { allowed, banned, forced };
	static const std::array<mode, 4> allModes;
	static std::string toStr(mode m);
	static mode fromStr(std::string txt);
	static const mode defaultMode;
...
struct channelWaveForm {
	...
	std::map<std::string, Expression> customData; // Added for timeAvg
};

// Optionally remove channelWave if unused
struct channelWave {
	std::vector<waveSignalForm> waveSigs;
	int phaseOption = 0;
	int initType = 0;
	std::string delim;
	std::vector<ViReal64> wave;
	std::map<std::string, Expression> customData; // Added for consistency
};
...
//};
//
//// Remove or align channelWave (if unused, delete; if used, add customData)
//struct channelWave {
//	std::vector<waveSignalForm> waveSigs;
//	int phaseOption = 0;
//	int initType = 0;
