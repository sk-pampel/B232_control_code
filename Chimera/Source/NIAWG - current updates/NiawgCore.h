...
class NiawgCore : public IDeviceCore {
	public:
		...
		void generateWaveform(channelWaveForm& chanWave, long int sampleNum, double waveTime, unsigned varnum,
			std::vector<parameterType>& variables,
		...
		void normalFinish ();
		void errorFinish ();
		void handleSpecialWaveform(NiawgOutput& output, std::string cmd, ScriptStream& script,
			std::vector<parameterType>& variables,
			std::vector<vectorizedNiawgVals>& vectorizedVals);
	...
	private:
		...
		const float NIAWG_GAIN = 1.36f; // NIAWG_GAIN = 1.36f;
		// NEW GAIN: 1.23f
		...
		void writeStandardWave(simpleWaveForm& wave, bool isDefault, unsigned varnum,
			std::vector<parameterType>& variables, niawgLibOption::mode libOption = niawgLibOption::defaultMode);
		void writeFlashing( waveInfoForm& wave, unsigned variation );

		// Wrapper overload
		void generateWaveform(channelWaveForm& chanWave, long int sampleNum, double waveTime, unsigned varnum,
			std::vector<parameterType>& variables, niawgWaveCalcOptions calcOpts = niawgWaveCalcOptions());
		...
		void createFlashingWave(waveInfoForm& wave, unsigned varnum, std::vector<parameterType>& variables);
		...
		void finalizeStandardWave(simpleWaveForm& wave, unsigned varnum, std::vector<parameterType>& variables,
			niawgWaveCalcOptions calcOpts = niawgWaveCalcOptions());
		/// member variables
		...
		void loadSpecialWave(NiawgOutput& output, std::string cmd, ScriptStream& script,
			std::vector<parameterType>& variables,
			simpleWaveForm& wave,
			std::vector<vectorizedNiawgVals>& vectorizedVals);

		void readSpecialSimpleWaveParams(ScriptStream& script, std::vector<parameterType>& parameters,
