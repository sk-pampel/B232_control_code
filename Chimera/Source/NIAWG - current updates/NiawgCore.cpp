...
void NiawgCore::programNiawg( std::string& warnings, unsigned variation, rerngGuiOptions& rerngGuiForm,
	...
	std::string niawgMachineScriptTest = std::string(niawgMachineScript.begin(), niawgMachineScript.end());
...
//		std::vector<parameterType> params; // Define params for consistency
//		analyzeNiawgScript(expOutput, warnings, rInfoDummy, params);
//		finalizeScript(expRepetitions, "DefaultNiawgScript", expOutput.niawgLanguageScript,
//			niawgMachineScript, !outputVaries(expOutput));
//		// Updated call to writeStaticNiawg with 6 arguments
//		writeStaticNiawg(expOutput, params, 0, 1, false, niawgLibOption::defaultMode);
...
//		programNiawg(warnings, 0, expRerngOptions, params, expOutput);
//	}
//	catch (ChimeraError&) {
...
void NiawgCore::setDefaultWaveforms() {
	...
	try {
		...
		finalizeScript(expRepetitions, "DefaultNiawgScript", expOutput.niawgLanguageScript,
			niawgMachineScript, !outputVaries(expOutput));
		writeStaticNiawg(expOutput, std::vector<parameterType>(), 0, 1, false, niawgLibOption::defaultMode);
		expOutput.niawgLanguageScript.insert(0, "script DefaultNiawgScript\n");
		expOutput.niawgLanguageScript += "end Script";
		defaultScript = niawgMachineScript;
...
void NiawgCore::restartDefault() {
	if (defaultMixedWaveform.empty() || defaultWaveName.empty()) {
		debugMsg += "WARNING: Default waveform not set or invalid. Skipping restart.\n";
		return; // Avoid NIAWG calls with invalid data
	}
	fgenFlume.writeNamedWaveform(cstr(defaultWaveName), static_cast<ViInt32>(defaultMixedWaveform.size()),
		defaultMixedWaveform.data()); // Line 283
	try	{
		// to be sure.
		turnOffRerng( );		
		turnOff();
...
void NiawgCore::programVariations(unsigned variation, std::vector<long>& variedMixedSize, NiawgOutput& output) {
	...
	for (unsigned waveInc = 0; waveInc < output.waveFormInfo.size(); waveInc++) {
		...
		if (output.waveFormInfo[waveInc].core.varies) {
			if (mixedWriteCount >= variedMixedSize.size()) {
				thrower("ERROR: mixedWriteCount=" + str(mixedWriteCount) +
					" exceeds variedMixedSize size=" + str(variedMixedSize.size()));
			}
			long waveSize = output.waveFormInfo[waveInc].core.waveVals.size();
			if (waveSize != variedMixedSize[mixedWriteCount]) {
				thrower("ERROR: waveVals size=" + str(waveSize) + " does not match variedMixedSize[" +
					str(mixedWriteCount) + "]=" + str(variedMixedSize[mixedWriteCount]));
			}
...
void NiawgCore::writeStaticNiawg(NiawgOutput& output, std::vector<parameterType>& variables, unsigned varnum,
	...
	for (auto waveInc : range(output.waveFormInfo.size())) {
		...
		debugMsg += "writeStaticNiawg: waveInc=" + str(waveInc) + ", name=" + waveForm.core.name +
			", varies=" + str(waveForm.core.varies) + "\n";
		if (waveForm.flash.isFlashing) {
			...
			if (!waveForm.core.varies) {
				...
				debugMsg += "Flashing waveform written: name=" + waveForm.core.name + "\n";
			}
		}
			...
			if (!waveForm.rearrange.staticWave.varies && !waveForm.rearrange.fillerWave.varies) {
				...
				calcOpts.libOpt = libOption;
				finalizeStandardWave(waveForm.rearrange.staticWave, varnum, variables, calcOpts);
				debugMsg += "Rearrangement static wave: name=" + waveForm.rearrange.staticWave.name + "\n";
				finalizeStandardWave(waveForm.rearrange.fillerWave, varnum, variables, calcOpts);
				debugMsg += "Rearrangement filler wave: name=" + waveForm.rearrange.fillerWave.name + "\n";
			}
			else {
			...
			if (!waveForm.core.varies) {
				...
				writeStandardWave(waveForm.core, output.isDefault, varnum, variables, libOption);
				debugMsg += "Standard wave written: name=" + waveForm.core.name + "\n";
				if (deleteWaveAfterWrite) {
					deleteWaveData(waveForm.core);
					debugMsg += "Deleted wave data: name=" + waveForm.core.name + "\n";
				}
			}
		}
	}
...
void NiawgCore::writeStandardWave(simpleWaveForm& wave, bool isDefault, unsigned varnum,
	std::vector<parameterType>& variables, niawgLibOption::mode libOption) {
	debugMsg += "writeStandardWave: varnum=" + str(varnum) + ", isDefault=" + str(isDefault) + "\n";

	niawgWaveCalcOptions opts;
	opts.libOpt = libOption;
	finalizeStandardWave(wave, varnum, variables, opts);

	debugMsg += "Waveform prepared: name=" + wave.name + ", size=" + str(wave.waveVals.size()) + "\n";
	if (wave.waveVals.empty()) {
		debugMsg += "ERROR: wave.waveVals is empty\n";
		thrower("ERROR: Waveform data is empty for variation " + str(varnum) + ". Cannot write to NIAWG.");
	}
	if (wave.waveVals.size() % 2 != 0) {
		debugMsg += "ERROR: wave.waveVals size is odd\n";
		thrower("ERROR: Waveform length (" + str(wave.waveVals.size()) + ") is not a multiple of 2 for variation " +
			str(varnum) + ". NIAWG requires even sample counts.");
	}

	debugMsg += "Attempting to delete waveform " + wave.name + "\n";
	try {
		fgenFlume.deleteWaveform(cstr(wave.name));
	}
	catch (ChimeraError & e) {
		debugMsg += "Delete failed (likely not allocated yet): " + str(e.what()) + "\n";
	}

	ViInt32 allocSize = static_cast<ViInt32>(wave.waveVals.size() / 2);
	debugMsg += "Allocating waveform " + wave.name + " with size=" + str(allocSize) + "\n";
	try {
		fgenFlume.allocateNamedWaveform(cstr(wave.name), allocSize);
		debugMsg += "Allocation succeeded for " + wave.name + "\n";
	}
	catch (ChimeraError & e) {
		debugMsg += "ERROR: Allocation failed: " + str(e.what()) + "\n";
		throw;
	}

	debugMsg += "Writing waveform " + wave.name + " with " + str(wave.waveVals.size()) + " samples\n";
	try {
		fgenFlume.writeNamedWaveform(cstr(wave.name), static_cast<ViInt32>(wave.waveVals.size()), wave.waveVals.data());
		debugMsg += "Write succeeded for " + wave.name + "\n";
	}
	catch (ChimeraError & e) {
		debugMsg += "ERROR: Write failed: " + str(e.what()) + "\n";
		throw;
	}
	...
	if (isDefault) {
		...
		debugMsg += "Set as default waveform\n";
	}
}
...
void NiawgCore::handleSpecialWaveform(NiawgOutput& output, std::string cmd, ScriptStream& script,
	...
	// Load waveform parameters using a dedicated function.
	loadSpecialWave(output, cmd, script, variables, wave, vectorizedVals);

	// Convert to waveInfoForm and store in output.
	output.waveFormInfo.push_back(toWaveInfoForm(wave));

	// Name the waveform properly.
	output.waveFormInfo.back().core.name = output.isDefault ? "Waveform0"
		: "Waveform" + str(output.waveFormInfo.size());

	// Append the relevant command to the NIAWG script.
	output.niawgLanguageScript += "generate " + output.waveFormInfo.back().core.name + "\n";
}

void NiawgCore::loadSpecialWave(NiawgOutput& output, std::string cmd, ScriptStream& script,
	std::vector<parameterType>& variables,
	simpleWaveForm& wave,
	std::vector<vectorizedNiawgVals>& vectorizedVals) {
	// Determine axis (Horizontal/Vertical) from command.
	int axis = (cmd.find("horizontal") != std::string::npos) ? Axes::Horizontal : Axes::Vertical;

	// Read and initialize waveform type.
	wave.chan[axis].initType = determineSpecialWaveType(cmd);

	// Read waveform parameters (specialized version).
	readSpecialSimpleWaveParams(script, variables, axis, wave);
}

void NiawgCore::readSpecialSimpleWaveParams(ScriptStream& script, std::vector<parameterType>& parameters,
	...
	for (int signal = 0; signal < int(wave.chan[axis].waveSigs.size()); signal++) {
		...
		switch ((wave.chan[axis].initType - 1) / NiawgConstants::MAX_NIAWG_SIGNALS) {
			// Special constant waveform (adjusted for time-averaged signals)
		case 0: {
			...
			break;
			  ...
			  // Special amplitude ramp waveform
		case 1: {
			...
			break;
			  ...
			  // Special frequency ramp waveform
		case 2: {
			...
			break;
			  ...
			  // Special frequency & amplitude ramp waveform
		case 3: {
			script >> sig.freqRampType >> sig.freqInit >> sig.freqFin >> sig.powerRamp.type
				>> sig.powerRamp.start >> sig.powerRamp.end >> sig.initPhase;
			sig.powerRamp.isRamp = true;
			assertAllValid(sig, parameters);
			break;
		}
...
//	}
//	else if (cmd == "timeavg") {
//		std::string channel, freq, upFreq, lowFreq, modFreq, amp, phase, delimiter;
//		script >> channel >> freq >> upFreq >> lowFreq >> modFreq >> amp >> phase >> delimiter;
//		if (delimiter != "#") {
//			thrower("Expected \"#\" but found \"" + delimiter + "\" in Niawg File during timeavg read");
//		}
//
//		// Use Axes::type instead of Axes
//		Axes::type targetAxis;
//		if (channel == "horizontal") {
//			targetAxis = Axes::Horizontal; // Now valid
//		}
//		else if (channel == "vertical") {
//			targetAxis = Axes::Vertical;   // Now valid
//		}
//		else {
//			thrower("Invalid channel \"" + channel + "\" for timeavg; must be \"Horizontal\" or \"Vertical\"");
//		}
//
//		Expression freqExpr(freq), upFreqExpr(upFreq), lowFreqExpr(lowFreq),
//			modFreqExpr(modFreq), ampExpr(amp), phaseExpr(phase);
//		std::string scope = NO_PARAMETER_SCOPE;
//		freqExpr.assertValid(variables, scope);
//		upFreqExpr.assertValid(variables, scope);
//		lowFreqExpr.assertValid(variables, scope);
//		modFreqExpr.assertValid(variables, scope);
//		ampExpr.assertValid(variables, scope);
//		phaseExpr.assertValid(variables, scope);
//
//		waveInfoForm wave;
//		wave.core.name = "TimeAvg_" + channel + "_" + str(output.waveFormInfo.size());
//
//		wave.core.chan[targetAxis].initType = -1; // Custom type
//		wave.core.chan[targetAxis].customData = {
//			{"freq", freqExpr}, {"upFreq", upFreqExpr}, {"lowFreq", lowFreqExpr},
//			{"modFreq", modFreqExpr}, {"amp", ampExpr}, {"phase", phaseExpr}
//		};
//
//		output.waveFormInfo.push_back(wave);
//		output.niawgLanguageScript += "generate " + wave.core.name + "\n";
//	}
//	else {
...
void NiawgCore::handleVariations(NiawgOutput& output, std::vector<parameterType>& variables, unsigned variation, unsigned totalVarNum,
	...
	unsigned totalVariations = ExpThreadWorker::determineVariationNumber(variables); // Fixed typo: totalVaraitions -> totalVariations
	rerngGuiOptionsFormToFinal(rerngGuiForm, variables, totalVariations);
	int mixedCount = 0;
	...
	for (auto waveInc : range(output.waveFormInfo.size())) {
		...
		if (waveForm.core.varies) {
			if (waveForm.flash.isFlashing) {
				flashFormToOutput(waveForm, variables, totalVarNum);
				writeFlashing(wave, variation); // Updated to pass variables (assuming prior fix)
			}
			else if (waveForm.rearrange.isRearrangement) {
				rerngScriptInfoFormToOutput(waveForm, variables, totalVarNum);
			}
				...
				writeStandardWave(wave.core, output.isDefault, variation, variables, niawgLibOption::defaultMode); // Line 617 - Fixed
				deleteWaveData(wave.core);
			}
			mixedWaveSizes.push_back(2 * wave.core.sampleNum(variation));
			mixedCount++;
		}
		// waveInc++ is unnecessary here; range() handles iteration
	}
...
void NiawgCore::generateWaveform(channelWaveForm& chanWave, long int sampleNum, double waveTime, unsigned varnum,
	std::vector<parameterType>& variables, niawgWaveCalcOptions calcOpts) {
	generateWaveform(chanWave, sampleNum, waveTime, varnum, variables, this->waveLibrary, calcOpts);
}
...
void NiawgCore::generateWaveform(channelWaveForm& chanWave, long int sampleNum, double waveTime, unsigned varnum,
	...
	if (chanWave.initType < 1) {
		if (chanWave.initType == -1 && chanWave.customData.find("modFreq") != chanWave.customData.end()) {
			std::chrono::time_point<chronoClock> time1(chronoClock::now());
			std::vector<ViReal64> readData(sampleNum);

			double freqVal = chanWave.customData["freq"].evaluate(variables, varnum);
			double upFreqVal = chanWave.customData["upFreq"].evaluate(variables, varnum);
			double lowFreqVal = chanWave.customData["lowFreq"].evaluate(variables, varnum);
			double modFreqVal = chanWave.customData["modFreq"].evaluate(variables, varnum); // Hz
			double ampVal = chanWave.customData["amp"].evaluate(variables, varnum);
			double phaseVal = chanWave.customData["phase"].evaluate(variables, varnum);

			double freqCenter = (lowFreqVal + upFreqVal) / 2;
			double freqAmp = (upFreqVal - lowFreqVal) / 2;

			for (long int i = 0; i < sampleNum; i++) {
				double t = i * waveTime / (sampleNum - 1); // Time in ms
				double freq = freqCenter + freqAmp * sin(2 * M_PI * modFreqVal * t / 1000); // modFreq in Hz, t in ms
				double phase = 2 * M_PI * freq * t / 1000 + phaseVal; // freq in MHz, t in ms
				readData[i] = ampVal * sin(phase);
			}

			chanWave.wave = std::vector<ViReal64>(readData.begin(), readData.end());
			readData.clear();
			...
			if (outputWriteTimes) {
				...
				debugMsg += "Finished generating timeavg waveform. Ellapsed Time: " + str(ellapsedTime) + " seconds.\r\n";
			}
			return;
		}
		else {
	...
	if (calcOpts.libOpt != niawgLibOption::mode::banned) {
		for (auto signal : range(chanWave.waveSigs.size())) {
			waveformFileSpecs += (str(chanWave.waveSigs[signal].freqInit.evaluate(variables, varnum)) + " "
				+ str(chanWave.waveSigs[signal].freqFin.evaluate(variables, varnum)) + " "
				+ chanWave.waveSigs[signal].freqRampType + " "
				+ str(chanWave.waveSigs[signal].powerRamp.start.evaluate(variables, varnum)) + " "
				+ str(chanWave.waveSigs[signal].powerRamp.end.evaluate(variables, varnum)) + " "
				+ chanWave.waveSigs[signal].powerRamp.type + " "
				+ str(chanWave.waveSigs[signal].initPhase.evaluate(variables, varnum)) + ", ");
		}
		...
		for (unsigned fileInc = 0; fileInc < waveLibrary[chanWave.initType - 1].size(); fileInc++) {
			if (waveLibrary[chanWave.initType - 1][fileInc] == waveformFileSpecs) {
				std::string waveFileReadName = LIB_PATH + WAVEFORM_TYPE_FOLDERS[chanWave.initType - 1] // Fixed
		...
		waveformFileName = (LIB_PATH + WAVEFORM_TYPE_FOLDERS[chanWave.initType - 1] // Fixed
			+ str(chanWave.initType) + "_" + str(waveLibrary[chanWave.initType - 1].size()) + ".txt");
	...
	if (!waveformFileWrite.is_open() && calcOpts.libOpt != niawgLibOption::mode::banned) {
		...
		std::chrono::time_point<chronoClock> time1(chronoClock::now());
		std::vector<ViReal64> readData(sampleNum + chanWave.waveSigs.size());
		calcWaveData(chanWave, readData, sampleNum, waveTime, varnum, calcOpts.powerOpt); // Fixed
		if (calcOpts.libOpt != niawgLibOption::mode::banned) {
		...
		std::fstream libNameFile;
		if (calcOpts.libOpt != niawgLibOption::mode::banned) {
			libNameFile.open(LIB_PATH + WAVEFORM_TYPE_FOLDERS[chanWave.initType - 1] // Fixed
...
void NiawgCore::calcWaveData( channelWaveForm& inputData, std::vector<ViReal64>& readData, long int sampleNum, 
	...
	for ( auto signal : range( inputData.waveSigs.size( ) ) ){ 
		...
		if ( inputData.waveSigs[signal].powerRamp.type != "lin" 
			 && inputData.waveSigs[signal].powerRamp.type != "nr"
		...
		if ( inputData.waveSigs[signal].freqRampType != "lin" && inputData.waveSigs[signal].freqRampType != "nr"
			 ...
			 && inputData.waveSigs[signal].freqRampType != "sinehalf"
			 && inputData.waveSigs[signal].freqRampType != "sinequarter"
	...
	for ( ; sample < sampleNum; sample++ ){
		...
		for ( auto signal : range( inputData.waveSigs.size( ) ) ){
			...
			else if (inputData.waveSigs[signal].freqRampType == "sinequarter") {
				// omega{t} = omega
				// phi = omega*t
				phasePos[signal] = 2 * PI * f_0 * t
					- (dOmega * t_r*2 / PI) * cos(PI * t / t_r/2)
					+ (dOmega * t_r*2 / PI)
					+ phi_0;
			}
			//else if (inputData.waveSigs[signal].freqRampType == "sinehalf") {
			//	// omega{t} = omega
			//	// phi = omega*t
			//	phasePos[signal] = 2 * PI * f_0 * t
			//		- (dOmega * t_r / PI) * cos(PI * t / t_r)
			//		+ (dOmega * t_r / PI)
			//		+ phi_0;
			//}
				...
				// phi = omega*t
				phasePos[signal] = 2 * PI * f_0 * t
					+ dOmega * .5 * (t - (t_r / PI) * sin(PI * t / t_r ))
					+ phi_0;
			}

			else if (inputData.waveSigs[signal].freqRampType == "cosinequarter") {
				// omega{t} = omega
				// phi = omega*t
				phasePos[signal] = 2 * PI * f_0 * t
					+ dOmega * (t - (t_r*2 / PI) * sin(PI * t / t_r/2))
					+ phi_0;
			}
	...
	for ( auto signal : range( inputData.waveSigs.size( ) ) ){
		double f_0 = inputData.waveSigs[signal].freqInit.getValue(varNum) * 1e6; // Initial frequency in Hz
		double omega_0 = 2 * PI * f_0; // Initial angular frequency
		double dOmega = deltaOmega[signal]; // Total change in angular frequency
		double t_r = waveTime; // Ramp duration in seconds
		double phi_0 = inputData.waveSigs[signal].initPhase.getValue(varNum); // Initial phase offset
		...
		if ( inputData.waveSigs[signal].freqRampType == "lin" ){
			...
			// Final phase calculation for the sine wave ramp
			phasePos[signal] = omega_0 * curTime
				- (dOmega * t_r * 2/ PI) * cos(PI * curTime / t_r / 2)
				+ (dOmega * t_r * 2 / PI)
				+ phi_0;
		}
		//else if (inputData.waveSigs[signal].freqRampType == "sinehalf") {
		...
		//		- (dOmega * t_r / PI) * cos(PI * curTime / t_r)
		//		+ (dOmega * t_r / PI)
		//		+ phi_0;
		//}
			...
			// phi = omega*t
			phasePos[signal] = 2 * PI * f_0 * curTime
				+ dOmega * 0.5 * (curTime - (t_r  / PI) * sin(PI * curTime / t_r ))
				+ phi_0;
		}
			...
			// phi = omega*t
			phasePos[signal] = 2 * PI * f_0 * curTime
				+ dOmega * (curTime - (t_r * 2 / PI) * sin(PI * curTime / t_r / 2))
				+ phi_0;
		}
...
void NiawgCore::mixWaveforms(simpleWaveForm& waveCore, bool writeThisToFile, unsigned varnum,
	std::vector<parameterType>& variables) {
	long sampleNum = waveCore.sampleNum(varnum);
	debugMsg += "mixWaveforms: sampleNum=" + str(sampleNum) + ", Vertical size=" +
		str(waveCore.chan[Axes::Vertical].wave.size()) + ", Horizontal size=" +
		str(waveCore.chan[Axes::Horizontal].wave.size()) + "\n";

	if (sampleNum <= 0) {
		debugMsg += "ERROR: Invalid sample number\n";
		thrower("ERROR: Sample number is invalid (" + str(sampleNum) + ") for variation " + str(varnum));
	}
	waveCore.waveVals.resize(2 * sampleNum);
	...
	debugMsg += "Mixed waveform: waveVals size=" + str(waveCore.waveVals.size()) + "\n";
...
//void NiawgCore::finalizeStandardWave(simpleWaveForm& wave, unsigned varnum, std::vector<parameterType>& variables,
//	niawgWaveCalcOptions calcOpts) {
//	generateWaveform (wave.chan[Axes::Vertical], wave.sampleNum(varnum), wave.time.getValue(varnum), varnum,
//		variables, waveLibrary, calcOpts);
//	generateWaveform (wave.chan[Axes::Horizontal], wave.sampleNum(varnum), wave.time.getValue(varnum), varnum,
//		variables, waveLibrary, calcOpts);
//}

void NiawgCore::finalizeStandardWave(simpleWaveForm& wave, unsigned varnum, std::vector<parameterType>& variables,
	niawgWaveCalcOptions calcOpts) {
	debugMsg += "finalizeStandardWave: varnum=" + str(varnum) + ", time=" + str(wave.time.getValue(varnum)) +
		", sampleNum=" + str(wave.sampleNum(varnum)) + "\n";

	generateWaveform(wave.chan[Axes::Vertical], wave.sampleNum(varnum), wave.time.getValue(varnum), varnum,
		variables, calcOpts);
	debugMsg += "Vertical channel: size=" + str(wave.chan[Axes::Vertical].wave.size()) + "\n";

	generateWaveform(wave.chan[Axes::Horizontal], wave.sampleNum(varnum), wave.time.getValue(varnum), varnum,
		variables, calcOpts);
	debugMsg += "Horizontal channel: size=" + str(wave.chan[Axes::Horizontal].wave.size()) + "\n";

	mixWaveforms(wave, false, varnum, variables);
	debugMsg += "After mixWaveforms: waveVals size=" + str(wave.waveVals.size());
	bool allZeros = true;
	for (auto val : wave.waveVals) {
		if (val != 0) {
			allZeros = false;
			break;
		}
	}
	debugMsg += ", all zeros=" + str(allZeros) + "\n";

	if (wave.waveVals.empty()) {
		debugMsg += "ERROR: wave.waveVals is empty\n";
		thrower("ERROR: finalizeStandardWave failed to generate waveform data for variation " + str(varnum));
	}
	if (wave.waveVals.size() % 2 != 0) {
		thrower("ERROR: Waveform length (" + str(wave.waveVals.size()) + ") is not even for variation " + str(varnum));
	}
	if (wave.name.empty()) {
		wave.name = "wave" + str(varnum);
		debugMsg += "Set default name: " + wave.name + "\n";
	}
	else {
		debugMsg += "Kept script-provided name: " + wave.name + "\n";
	}
...
void NiawgCore::createFlashingWave(waveInfoForm& wave, unsigned varnum, std::vector<parameterType>& variables) {
	/// Quick check
	...
	niawgWaveCalcOptions calcOpts; // Default options
	for (unsigned waveInc = 0; waveInc < wave.flash.flashNumber; waveInc++) {
		finalizeStandardWave(wave.flash.flashWaves[waveInc], varnum, variables, calcOpts); // Line 1961 - Fixed
	}
	double staticMovingRatio = 1;
	if (wave.flash.flashWaves.size() <= 2) {
		staticMovingRatio = wave.flash.flashWaves.back().time.getValue(varnum) * 1e-3 /
			wave.flash.flashWaves.front().time.getValue(varnum) * 1e-3;
	}
...
bool NiawgCore::isSpecialWaveform(std::string command) {
	return (command == "flash" || command == "stream" || command == "rearrange" || command == "timeavg");
}
...
double NiawgCore::rampCalc (int totalSamples, int iteration, const rampInfo& ramp, unsigned varNum) {
	...
	if ( ramp.type == "lin" ){
		...
		return rampRange * 0.5 * (1 - cos(PI * (double)iteration / totalSamples));
	}
		...
		return rampRange * (sin(0.5 * PI * (double)iteration / totalSamples));
	}
		...
		return rampRange * (1-cos(0.5 * PI * (double)iteration / totalSamples));
	}
...
void NiawgCore::calculateVariations(std::vector<parameterType>& params, ExpThreadWorker* threadworker) {
	...
	if (experimentActive) {
		unsigned totalVariations = ExpThreadWorker::determineVariationNumber(params);
		...
		boost::replace_all(debugStr, "\n", "\r\n");
		notify({ qstr(debugStr), 2 }, threadworker);
		writeStaticNiawg(expOutput, params, 0, totalVariations, false, niawgLibOption::defaultMode);
...
//			!outputVaries(expOutput));
//		// ... (debug output) ...
//		writeStaticNiawg(expOutput, params, 0, totalVariations, false, niawgLibOption::defaultMode); // Reverted to false
//		emit threadworker->warn({ str(niawgWarnings), 1, getDelim() });
//		notify({ qstr(getOutputSummary(expOutput)), 2 }, threadworker);
//	}
//}
...
//void NiawgCore::programVariation(unsigned variationInc, std::vector<parameterType>& expParams,
//	ExpThreadWorker* threadworker) {
//	debugMsg += "programVariation: variationInc=" + str(variationInc) + "\n";
//	try {
//		// Assuming this programs the script
//		debugMsg += "Programming NIAWG with script: " + std::string(niawgMachineScript.begin(), niawgMachineScript.end()) + "\n";
//		fgenFlume.writeScript(niawgMachineScript); // Adjust based on actual call
//		debugMsg += "Script programmed successfully\n";
//	}
//	catch (ChimeraError & e) {
//		debugMsg += "ERROR in programVariation: " + str(e.what()) + "\n";
//		throw;
//	}
//}
